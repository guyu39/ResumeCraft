...
7. **字体编码检测：** 优化用户粘贴体验，自动识别非标准化字体样式并尝试修复。
9. **自动保存**：localStorage 防抖持久化，刷新后可恢复。
10. **云端同步**：登录后自动同步到云端，支持跨端恢复。
11. **A4 分页预览**：按页切片展示，减少内容截断。
12. **AI 辅助**：简历评估、内容润色建议。
13. **PDF 导出**：后端异步任务模式（创建任务 → 轮询状态 → 下载）。
