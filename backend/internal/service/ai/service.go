...
func buildEvaluatePrompt(content map[string]interface{}) string {
	...
	sb.WriteString("4. 无效/占位内容识别（极重要，禁止当作有效内容计分）：以下均视为「无效内容」，必须等同于「未填写」处理，并在对应 issue 中明确指出：\n")
	sb.WriteString("   - 重复字符或键盘乱敲：如「11111」「aaaa」「测试测试测试」「asdf」「。。。」；\n")
	sb.WriteString("   - 明显的占位/测试文本：如「测试」「demo」「示例」「待填写」「xxx」「下侧」「11：111111」，以及与字段语义不符的随意填写；\n")
	sb.WriteString("   - 未替换的模板提示语：如「请填写自我评价」「请输入...」「在此填写」等系统默认占位文案，必须判定为该模块未填写；\n")
	sb.WriteString("   - 字段值与字段名严重不符：如姓名填「11111」、公司名填「测试」、项目名填「测试」、角色填「下侧」。\n")
	sb.WriteString("5. 含无效内容的模块按「未有效填写」对待：该模块不得贡献正分，并必须在 issue_item 中以 medium 及以上严重程度报出「占位/无效内容」问题。\n")
	sb.WriteString("6. 内容可信度审查：若某模块（尤其项目/工作经历）内容详实但与简历整体画像（求职意向、教育、其他经历、所列技能）明显割裂或不匹配（例如个人信息与经历全是占位测试文本，却出现一段高度完整、强量化的项目描述），应在 issue 中提示「内容真实性/一致性存疑」，并在「整体印象」「量化成果」维度审慎给分，不得因单段华丽文本而抬高综合分。量化数据（如「降低70%」「提升3-5倍」）若无上下文支撑，视为未经验证的自述，不得据此显著加分。\n")
	...
	sb.WriteString("5. 占位/无效内容的扣分红线：若个人信息、教育、工作/项目中存在第4条所列的占位/乱填内容，相关维度（结构完整性、内容相关性、整体印象）必须显著扣分；当核心模块普遍为占位内容时，综合分不得超过 40 分（定级 C 及以下），禁止因个别模块文字华丽而给出 60+ 高分。\n")
	sb.WriteString("6. 未填写模块（如自我评价为空、技能未填）按缺失计，不得默认给中性分；缺失越多，结构完整性维度越低。\n")
